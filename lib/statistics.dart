/// Statistics library.
library statistics;

export 'package:data_serializer/data_serializer.dart';

export 'src/statistics_base.dart';
export 'src/statistics_bayesnet.dart';
export 'src/statistics_benchmark.dart';
export 'src/statistics_combination.dart';
export 'src/statistics_decimal.dart';
export 'src/statistics_dynamic_int.dart';
export 'src/statistics_equality.dart';
export 'src/statistics_equality_simd.dart';
export 'src/statistics_extension.dart';
export 'src/statistics_extension_num.dart';
export 'src/statistics_extension_simd.dart';
export 'src/statistics_forecast.dart';
export 'src/statistics_metric.dart';
export 'src/statistics_prime.dart';
export 'src/statistics_tools.dart';
export 'src/statistics_tools_csv.dart';
